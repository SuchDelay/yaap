...
class YouTubeTUI:
    ...
    def draw_header(self):
        ...
        title = "♪ YAAP - Yet Another Audio Player ♪"
        self.stdscr.attron(curses.color_pair(1) | curses.A_BOLD)
        self.stdscr.addstr(0, max(0, (width - len(title)) // 2), title)
        self.stdscr.attroff(curses.color_pair(1) | curses.A_BOLD)
        
        mode = "AUDIO" if self.audio_only else "VIDEO"
        cava_status = "CAVA:ON" if self.has_cava else "CAVA:N/A"
        lyrics_status = "LYRICS:ON" if self.show_lyrics else "LYRICS:OFF"
            
        status = f"Mode: {mode} | {cava_status} | {lyrics_status}"
        if len(status) + 2 < width:
            self.stdscr.addstr(1, width - len(status) - 2, status, curses.color_pair(3))
    ...
    def draw_search_box(self):
        ...
        if self.search_mode:
            ...
            self.stdscr.addstr(3, 11, display_text[:width - 13], curses.color_pair(3) | curses.A_BOLD)
        else:
            display_query = self.search_query if self.search_query else "[Click or press 's' to search]"
            self.stdscr.addstr(3, 11, " " * (width - 13))
            self.stdscr.addstr(3, 11, display_query[:width - 13], curses.color_pair(6))
    ...
    def draw_thumbnail(self, y_start, x_start, video_id):
        ...
        try:
            for i, line in enumerate(thumb_lines[:6]):
                if y_start + i < height - 2 and len(line) > 0 and x_start < width:
                    self.stdscr.addstr(y_start + i, x_start, line[:max(0, width - x_start - 1)], curses.color_pair(1))
        except:
            pass
    ...
    def draw_results(self):
        ...
        if not self.results:
            if self.search_query:
                self.stdscr.addstr(6, 2, "No results found.", curses.color_pair(4))
            else:
                self.stdscr.addstr(6, 2, "Search for music or videos!", curses.color_pair(6))
            return
        ...
        for i in range(start_idx, end_idx):
            ...
            if title_x < width:
                if is_selected:
                    ...
                    self.stdscr.addstr(y_pos, title_x, f"{prefix}{title}"[:max(0, results_width - 46)])
                ...
                if y_pos + 1 < height:
                    self.stdscr.addstr(y_pos + 1, title_x, info[:max(0, results_width - 46)], curses.color_pair(6))
            
            y_pos += 8
    
    def draw_cava_visualizer(self):
        """Draw integrated Cava visualizer"""
        ...
        try:
            ...
            if 7 < height:
                self.stdscr.addstr(7, viz_x, "╔" + "═" * (viz_width - 2) + "╗")
            for i in range(1, viz_height - 1):
                if 7 + i < height - 1:
                    self.stdscr.addstr(7 + i, viz_x, "║" + " " * (viz_width - 2) + "║")
            ...
            if self.cava_output:
                for i, line in enumerate(self.cava_output[:viz_height - 2]):
                    if 8 + i < height - 2:
                        self.stdscr.addstr(8 + i, viz_x + 1, line[:max(0, viz_width - 2)], curses.color_pair(2))
        except:
            pass
        ...
        try:
            ...
            for i in range(1, lyrics_height - 1):
                if lyrics_y + i < height - 6:
                    self.stdscr.addstr(lyrics_y + i, lyrics_x, "║" + " " * (lyrics_width - 2) + "║")
            ...
            if self.lyrics:
                ...
                for i, line in enumerate(self.lyrics[start_line:start_line + lyrics_height - 2]):
                    if lyrics_y + 1 + i < height - 6:
                        ...
                        color = curses.color_pair(3) | curses.A_BOLD if is_current else curses.color_pair(6)
                        text = line[:max(0, lyrics_width - 3)].center(max(0, lyrics_width - 3))
                        self.stdscr.addstr(lyrics_y + 1 + i, lyrics_x + 1, text, color)
            else:
    ...
    def draw_now_playing(self):
        ...
        if self.current_video:
            ...
            title = self.current_video.get('title', 'Unknown')[:max(0, width // 2 - 6)]
            if y_pos + 1 < height:
                self.stdscr.addstr(y_pos + 1, 2, title, curses.color_pair(3))
            
            status = "Playing" if self.playing else "Stopped"
            if y_pos + 2 < height:
                self.stdscr.addstr(y_pos + 2, 2, f"Status: {status}", curses.color_pair(2))
    ...
    def draw_help(self):
        ...
        for i, text in enumerate(help_text):
            if y_pos + i < height:
                self.stdscr.addstr(y_pos + i, 2, text[:max(0, width-4)], curses.color_pair(4))
            ...
            try:
                ...
                for entry in entries[:10]:
                    ...
                    video_data = {
                        ...
                        'thumbnail': entry.get('thumbnail', f'https://i.ytimg.com/vi/{video_id}/default.jpg')
                    }
    ...
    def search_youtube_fallback(self, query: str):
        """Fallback search method if fast search fails"""
        try:
            cmd = [
                'yt-dlp',
                '--get-id',
    ...
    def fetch_lyrics(self, title: str):
        ...
        - No API key required
        - We use /api/search?q=<query> and take the first match
        """
        self.lyrics = []
        self.current_lyric_line = 0
    ...
    def update_cava_output(self):
        """Read cava raw ascii output and update visualizer buffer"""
...
"""
    ...
    def play_video(self, video: Dict):
        """Play video/audio using mpv"""
        if self.mpv_process:
            self.stop_playback()
            
        try:
            cmd = ['mpv', '--volume=100', '--really-quiet', '--no-terminal']
            if self.audio_only:
                cmd.append('--no-video')
        ...
        try:
            ...
            if self.results and y >= 7:
                ...
                if 0 <= result_idx < len(self.results):
                    if result_idx == self.selected_index or (bstate & curses.BUTTON1_CLICKED):
        ...
        if self.search_mode:
            if key == 27:
                ...
                if self.search_input.strip():
                    ...
                    self.results = self.search_youtube_fast(self.search_query)
                    self.selected_index = 0
                return True
...
def main():
    ...
    try:
        subprocess.run(['cava', '--version'], capture_output=True, timeout=1)
        print("✓ Cava detected")
    except:
        print("ℹ Cava not found - install with: sudo pacman -S cava")
    
    try:
        subprocess.run(['jp2a', '--version'], capture_output=True, timeout=1)
